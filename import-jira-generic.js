...
const PROJECT_KEY = process.env.PROJECT_KEY || "SOPS";
const QUEUE_NAME  = process.env.QUEUE_NAME  || "SalesOps";
...
async function ensureUser(jiraUser) {
  ...
  if (existing.rows.length > 0) {
    userCache[email] = existing.rows[0].id;
    return existing.rows[0].id;
  }
  // Insert with usr_ prefix id
  ...
  const finalId = row.rows[0]?.id || null;
  if (finalId) userCache[email] = finalId;
  return finalId;
}
...
async function main() {
  ...
  console.log(`Importing Jira project "${PROJECT_KEY}" â†’ queue "${QUEUE_NAME}"`);

  console.log("Fetching statuses from Jira...");
  ...
  console.log(`Total issues from Jira: ${issues.length}`);
